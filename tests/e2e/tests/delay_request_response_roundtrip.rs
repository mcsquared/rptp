...
mod tests {
    ...
    #[tokio::test]
    #[ignore]
    async fn delay_request_response_roundtrip() -> anyhow::Result<()> {
        ...
        let master = TestContainer::new(
            GenericImage::new(image.name(), &tag)
                ...
                .with_cmd(["/app/delay-request-response-roundtrip-master"])
        ...
        let slave = TestContainer::new(
            GenericImage::new(image.name(), &tag)
                ...
                .with_cmd(["/app/delay-request-response-roundtrip-slave"])
                .with_network(net.name())
                .start()
                .await?,
            docker.clone(),
