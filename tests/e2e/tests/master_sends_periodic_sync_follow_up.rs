...
mod tests {
    ...
    #[tokio::test]
    #[ignore]
    async fn master_sends_periodic_sync_follow_up() -> anyhow::Result<()> {
        ...
        let slave = TestContainer::new(
            GenericImage::new(image.name(), &tag)
                ...
                .with_cmd(["/app/master-sends-periodic-sync-follow-up-slave"])
        ...
        let master = TestContainer::new(
            GenericImage::new(image.name(), &tag)
                ...
                .with_cmd(["/app/master-sends-periodic-sync-follow-up-master"])
                .with_network(net.name())
                .start()
                .await?,
            docker.clone(),
